...
WINDOW_CONF = {"layer": 0.80, "block_keyword": 0.65, "block_geometry": 0.45, "thin_lines": 0.05}  # ağırlık turu 4 (2026-09-14): weights.yaml window ile aynı (geçiş fallback)
    ...
    for f1 in b1.floors:
        ...
        for i, r in enumerate(f1.rooms):
            ...
            fl.rooms.append(Room(
                id=rid, confidence=conf, evidence=ev,
        ...
        wlay = list(getattr(f1, "wall_layers", []) or [])
        for i, (a, b) in enumerate(f1.walls):
            ...
            fl.walls.append(Wall(id=f"w{i + 1}", confidence=conf, evidence=ev, thickness=thk,
                                 layer=(wlay[i] if i < len(wlay) else None),
