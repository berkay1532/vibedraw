...
| 22 | Çelişen sinyal: A_ANNO_* / A_STAIR_* katmanındaki çizgiler adına göre yazı/merdiven, geometrisine göre duvar çifti (ANTRE–HOL bölmesi, DEPO duvarı) | tip-1 (HOL kaybı) vs tip-4 (ANTRE doğru birleşti) | sözlük sınıfı hariç tutunca tip-1 bozuldu, tip-4 düzeldi → hariç tutma profil güvenine kapılandı | **conflicting_signal** adayı: "Bu çizgi grubu duvar mı, açıklama mı?" — Adım 6'da ÖZEL KURAL DEĞİL, genel sinyal: `parallel_pair` katman sınıfından bağımsız her segment için hesaplanır, `layer_class` ayrı sinyal; çelişki → düşük güvenli duvar + conflicting_signal issue | duvar / açıklama-yazı / merdiven / yoksay |
| 21 | Duvar bandındaki ince paralel çizgi grubu (`window:thin_lines`): 7 GT dosyasında 9 aday, 9'u sahte | çoğu hafif çelik 70 | pencere sanıldı (güven 0.3'e çekildi) | **ambiguous_opening** adayı: "Bu ince paralel çizgi grubu ne?" — Adım 7'de soru üretir, karar vermez | pencere / kalorifer / dolap / korkuluk / kaplama / yoksay |
| 23 | Etiketsiz kapalı bölge: duvar grafı (polygonize) yüzü hiçbir flood-fill odasıyla örtüşmüyor (çekirdek: merdiven/asansör/aydınlık/şaft; etiketsiz balkon) | src02-07 (8 aday), src02-09 (6), KAYAPINAR (4), tip-2/tip-4 (2–3) | `unlabeled_region` issue + `raw_name=""` aday mahal (güven 0,45), kapıya bağlanmaz | "Bu alan?" | merdiven / asansör / aydınlık / şaft / balkon / oda / yoksay |
